...
ctext = bytes.fromhex(args.ciphertext)
ctext_len = len(ctext)
...
cribVsPos={'byuctf{n3v3r_g0nn4_r3us3_4_k3y}':{'pos':[0,31,93,124,155,186,217,248,279,310,341,403,465,496,558,589,620,682,713,744,806,868,899,930,1023,1116,1178,1209,1240,1302,1333,1364,1488,1612,1705,1736],
											  'message':'m'}}
for i in cribVsPos:
	known_prefix=i
	known_prefix_length=len(known_prefix)
	for k in cribVsPos[i]['pos']:
		known_results = sxor(ctext, known_prefix)
		if cribVsPos[i]['message'] == 'm':
			display_ctext = display_ctext[:k] + known_prefix + display_ctext[k+known_prefix_length:]
			display_key = display_key[:k] + known_results[k] + display_key[k+known_prefix_length:]
		else:
			display_key = display_key[:k] + known_prefix + display_key[k+known_prefix_length:]
			display_ctext = display_ctext[:k] + known_results[k] + display_ctext[k+known_prefix_length:]
